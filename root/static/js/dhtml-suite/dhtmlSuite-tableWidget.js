...
// Default value for widthOfTable added for local convenience.
        // Internet explorer hack.
        if ( navigator.appName.match(/Explorer/i) ) {
            this.widthOfTable=800;
        }
        else {
            this.widthOfTable=850;
        }
...
// {{{ __initTableWidget()
	...
	*Initializes the table widget script. This method formats the table and add events to the header cells.
	...
	if(!this.noCssLayout)tBody.className='DHTMLSuite_scrollingContent';
        if(tBody.offsetHeight>(tBody.parentNode.parentNode.offsetHeight - 30)) {
