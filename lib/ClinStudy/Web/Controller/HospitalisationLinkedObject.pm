...
package ClinStudy::Web::Controller::HospitalisationLinkedObject;
...
ClinStudy::Web::Controller::HospitalisationLinkedObject - Catalyst Controller abstract class
...
# Specific to container base subclasses.
sub BUILD {

    my ( $self, $params ) = @_;

    $self->my_container_namespace( 'hospitalisation' );
...
=head2 list_by_hospitalisation 

=cut

sub list_by_hospitalisation : Local {
    my $self = shift;
...
sub _set_my_breadcrumbs {
    ...
    if ( $container_field && $object ) {
        ...
        my $patient         = $hospitalisation->patient_id();
